...
class Database
{
	...
	// Get uid
	public function uid($table, $id)
	{
		$uid = $this->select_one($table, 'uid', ['id' => $id], ['default' => null]);
		return $uid;
	}
	// Update Uid
	...
	{
		$uid = getRand(30);
		$query = "UPDATE $table SET uid = '$uid' WHERE id = $insert_id";
		$update = false;
		try {
			$update = $this->query($query);
		} catch (Exception $e) {
	...
	public function assoc_to_simple($array, $key)
	{
		$new_array = [];
