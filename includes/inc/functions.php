...
// Apply svg icon
function svg_icon($name, $folder_dir = 'svg')
{
    global $dir;
    $svg = file_get_contents(merge_path($dir, "images/$folder_dir/$name.svg"));
    $svg = str_replace("<svg ", '<svg class="sf-svg-icon" ', $svg);
    return $svg;
}

// properties panel heading
function cnPropPanelHeading($option = [])
{
    $iconClass = array_key_exists('icon-class', $option);
    $iconClass = ($iconClass) ? $option['icon-class'] : '';
    require_once "./components/panel-header.php";
}
...
function minify_files($files, $options = [])
{
    if (ENV != 'prod') return $files;
    $is_global_files = arr_val($options, 'is_global_files');
    $path = $options['path'];
    $type = $options['type'];
    if (!in_array($type, ['css', 'js'])) return $files;
    $source_dir = merge_path(_DIR_, 'source');
    @mkdir($source_dir);
    // Generate filename
    $assets_v = explode("=", ASSETS_V);
    $assets_v = end($assets_v);
    $assets_v = md5($assets_v);
    $file_param = '';
    // Check global file
    if (!$is_global_files) {
        $current_filename = basename($_SERVER['SCRIPT_FILENAME']);
        $current_filename = explode('.', $current_filename);
        array_pop($current_filename);
        $file_param = "-" . implode('-', $current_filename);
    }
    $file_type_name = $type == "js" ? "script" : "styles";
    $filename = "{$file_type_name}{$file_param}-" . $assets_v . ".min." . $type;
    $filepath = merge_path($source_dir, $filename);
    ...
    foreach ($files as $file) {
        $attach_path = $path;
        // Check if file has already full path
        if (strstr($file, './')) $attach_path = '';
        // Check if file is from cdn
        if (strstr($file, 'http')) {
            if (!strstr($file, SITE_URL))
                array_unshift($output_files, $file);
            continue;
        }
        // Attach path
        $file = merge_path($attach_path, $file);
        ...
        if (!is_file($file)) {
            echo '<script>console.error(' . $file . ' not found! 404)</script>';
            continue;
        }
        $minifier->addFile($file);
    }
    // put content to minify file
    $done = $minifier->minify($filepath);
    return $done ? $filepath : $files;
}
