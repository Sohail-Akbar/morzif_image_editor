...
$.fn.isAddHTML = function () {
    return $(this).attr("data-toggle") === "addHTML";
}
$.fn.addHTML = function (type, data = {}) {
    if (!$(this).isAddHTML()) return false;
    if (type == "getPick") {
        let $pick,
        ...
        let $drop = $(this).addHTML('getDrop'),
            $pick = $(this).addHTML('getPick');
        if (!$drop || !$pick) return false;
        return true;
    } else if (type === "addItems") {
        if (!$(this).addHTML('isValid')) return false;
        // Start filling data
        let $drop = $(this).addHTML("getDrop"),
            $pick = $(this).addHTML("getPick");
        $drop.html('');
        for (let i = 0; i < data.itemsCount; i++) {
            $(this).trigger("click");
        }
    }
...
// tc folding card
$(document).on('change', ".tc-folding-card .card-header .card-toggler", function (e) {
    let $parent = $(this).parents('.tc-folding-card'),
        $this = $parent.find(".card-header");
    // Add Class
    $this.toggleClass('active');
    if ($this.hasClass('active')) {
        $parent.find(`.tc-collapse`).slideDown()
    } else {
        $parent.find(`.tc-collapse`).slideUp()
    }
});
// #region TC Select
// init tc select
function initTcSelect() {
    $(".tc-select").each(function () {
        let $select = $(this),
            isMultiple = toBoolean($select.dataVal("multiple"));
        // Check if already launched
        if ($(this).hasAttr("data-select-launched")) return true;
        $(this).attr("data-select-launched", "true");
        // Options
        $(this).find(".tc-option").each(function () {
            // Label HTML of option
            let html = $(this).html();
            $(this).html(`
            <div class="tc-option-content">${html}</div>
            `);
            // Click Event on option
            $(this).on("click", function (e) {
                e.preventDefault();
                let $parent = $(this).parents(".tc-select").first();
                if (isMultiple) {
                    $(this).toggleClass("active");
                } else {
                    $parent.find(".tc-option.active").removeClass("active");
                    $(this).addClass("active");
                }
                refreshTcSelectOptions($parent);
            });
        });
        // Make Active
        if ($(this).find(".tc-option.active").length < 1) {
            if (!isMultiple)
                $(this).find(".tc-option").first().addClass("active");
        }
        // Prevent dropdown hidden on click
        $(this).find(".dropdown-menu").on("click", function (e) {
            $(this).parent().is(".show") && e.stopPropagation();
        });
        refreshTcSelectOptions($select, false);
    });
}
// Refresh TC Select
function refreshTcSelectOptions($select, isByClick = true) {
    let isMultiple = toBoolean($select.dataVal("multiple")),
        $input = $select.find(".tc-select-input");

    $select.find(".tc-option").each(function () {
        let $this = $(this),
            isActive = $this.hasClass("active");
        if (!isActive)
            return true;
        let value = $this.dataVal("value", "");
        if (isMultiple)
            return true;
        // Append Value
        $input.val(value);
        ...
        // Make option active
        $select.find(".tc-option.active").removeClass("active");
        $this.addClass("active");
        // Append HTML as selected element
        let html = $this.find(".tc-option-content").first().html();
        $select.find(".tc-selected-content").html(
            `<div class="tc-selected-item">${html}</div>`
        );
    });
    // Hide Select Option
    if (isByClick && !isMultiple) {
