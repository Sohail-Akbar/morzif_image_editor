...
(function (global, factory) {
  ...
  var hex = {
    init: function init(api) {
      var template = '<div class="clr-picker-con-2"><input type="text" class="' + api.namespace + '-hex" /><button class="clr-picker-icon"><img src="images/editor-icon/color-picker-cursor.svg"></button></div>';
      this.$hex = $(template).appendTo(api.$dropdown);
      this.$hex = this.$hex.find('.' + api.namespace + '-hex');

      this.$hex.on('change', function () {
        api.set(this.value);
        api._trigger("change", this.value);
      });
        ...
        },
        {
          ...
          value: function createMask() {
            ...
            $("body").on(
              ...
              function (e) {
                if (isEventTarget(e, '.asColorPicker-dropdown, .clr-picker-prevent'))
                  return true;
